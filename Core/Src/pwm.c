...
void initTIM(TIM_TypeDef* TIMx, uint16_t dutycycle, uint32_t frequency)
{
    ...
    DEBUGLOG("initialising PWM\r\n");
    DEBUGLOG("Requested values\r\n");
    DEBUGLOG("dutycycle: %d\t frequency: %d\r\n", dutycycle, frequency);
    DEBUGLOG("\nActual\r\n");
    DEBUGLOG("dutycycle1: %d\tdutycycle2: %d\t frequency: %d Hz\r\n", ((ccr1*100)/autoreload), ((ccr2*100)/autoreload), ((F_CPU)/((autoreload+1)*(prescal+1))) );
    DEBUGLOG("Debug: PSC: %d\tARR: %d\tCCR1: %d\tCCR2: %d\r\n", prescal, autoreload, ccr1, ccr2);
