...
void ssd1306_I2C_Init() {
	...
	I2C1->CCR |= 0x800e;//(1 << I2C_CCR_FS_Pos) | (1 << I2C_CCR_DUTY_Pos) | (0x02 << I2C_CCR_CCR_Pos);
...
void ssd1306_I2C_WriteMulti(uint8_t address, uint8_t reg, uint8_t* data, uint16_t count) {
	...
	while (!(I2C1->SR1 & (1<<0))); // wait for start bit to go high

	I2C1->DR = address;  //  send the address
	while (!(I2C1->SR1 & (1<<1)));  // wait for ADDR bit to set
	int temp = I2C1->SR1 | I2C1->SR2;  // read SR1 and SR2 to clear the ADDR bit
